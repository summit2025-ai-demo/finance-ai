    ...
    public Uni<Void> consume(Record<String, String> message) {
        ...
        return Uni.createFrom().item(message).emitOn(Infrastructure.getDefaultWorkerPool())
                .onItem().invoke(m -> {
                    ...
                    request.put("customer_id", json.getJsonObject("structured").getString("customer_id"));
                    request.put("request", json.getString("content"));
                    FinanceRequestResponse response = messageProcessor.process(request.encodePrettily());
                    json.put("finance", response.toJson());
                    if (!response.toolInvoked()) {
                        handleError(m.key(), json, "Finance: no tool invoked", null, null);
                        return;
                    }
